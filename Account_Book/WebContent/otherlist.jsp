	...
	<div class="container">
		<div class = "row">
			<table class="table table-striped" style="text-align:center; border:1px solid #dddddd"> 
				<thead>
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">수입</th>
						<th style="background-color: #eeeeee; text-align: center;">지출</th>
						<th style="background-color: #eeeeee; text-align: center;">통합</th>
					</tr>
				</thead>
				<tbody>
					<%
					...
					%>
					<tr>
						<td><%= list.get(i).getTotalid() %></td>				
						<td><%= list.get(i).getId() %></td>
						<td><a href = "inputlist.jsp?inputID=<%= list.get(i).getTotalid()%>"><%= list.get(i).getInputmoney()%></a></td>
						<td><a href = "outputlist.jsp?outputID=<%= list.get(i).getTotalid()%>"><%= list.get(i).getOutputmoney() %></a></td>
						<td><%= total %></td>
					</tr>
					<%
							}else if(total >= min && max == 0){
					%>
					<tr>
						<td><%= list.get(i).getTotalid() %></td>				
						<td><%= list.get(i).getId() %></td>
			...
			<%
				if(pageNumber != 1){
			%>	
				<a href = "otherlist.jsp?pageNumber=<%=pageNumber - 1%>" class = "btn btn-success btn-arraw-left">이전</a>
			<%
				} if(totalDAO.nextPage(pageNumber + 1)){
			%>
				<a href = "otherlist.jsp?pageNumber=<%= pageNumber + 1 %>" class = "btn btn-success btn-arraw-left">다음</a>
			<%
				}
			%>
		</div>
